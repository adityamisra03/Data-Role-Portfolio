# -*- coding: utf-8 -*-
"""
Created on Tue Oct  8 11:17:49 2024
...
os.getcwd()
os.chdir("C:/Users/adi03/Documents/School/BME 524/gbm_data")
...
with PdfPages('figures/scatter_total_counts_pct_mt_with_cutoffs.pdf') as pp:
  ...
  ax1.axvline(x=500, color = 'red', linestyle='--')
  ax1.axvline(x=70000, color = 'red', linestyle='--')
  pp.savefig()
  plt.show()
  plt.close()

# Remove cells that have too many mitochondrial genes expressed or too many total counts
keep = (adata.obs['pct_counts_mt']> 0.001) & (adata.obs['pct_counts_mt'] < 19) & (adata.obs['total_counts'] > 500) & (adata.obs['total_counts'] < 70000)
print("Removed cells: %d"%(adata.n_obs - sum(keep)))

keep = (adata.obs['pct_counts_mt'] < 17) & (adata.obs['total_counts'] < 70000) # show that total counts & genes are directly related; therefore, you really only need one cut-off
print("Removed cells: %d"%(adata.n_obs - sum(keep)))
...
# Write out CSV file with all marker gene details
result = adata.uns['rank_genes_groups']
groups = result['names'].dtype.names
df = pd.DataFrame({group + '_' + key: result[key][group] for group in groups for key in ['names', 'logfoldchanges', 'pvals', 'pvals_adj']})
df.to_csv('rank_genes.csv')
...
marker_genes_dict = {'Endothelial cells': ['CD34', 'VWR'],
                    'Mural cells': ['RGS5','PDGFRB'],
                    'T-cells': ['CD3D','CD3E'],
                    'B-cells': ['CD79A','IGHG1'],
                    'Dendritic cells': ['CCR6', 'HLA-DPB1'],
                    'Macrophage': ['APOC1', 'CCL20'],
                    'Neuronal/glial cells': ['FABP7', 'GFAP', 'GAPDH'],
                    'Cell cycle': ['CCNA2', 'CCNB1', 'PCNA', 'TOP2A'],
                    'Hypoxia': ['VEGFA', 'NDRG1']}
